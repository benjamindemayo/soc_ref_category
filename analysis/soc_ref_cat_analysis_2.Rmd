  ...
  #Take one eye if we only have one; otherwise average them
    d <-
      ...
      mutate(
        ...
        y = y.max - y #y flip so origin is cartesian
      )
}
```
...
```{r}
#Visualizing looking proportion by accuracy
...
```{r}
#Looking at a more constrained AOI
...
morph_proportions_constrained <- 
  trial_proportions_constrained %>% 
  group_by(Morph) %>% 
  multi_boot_standard(col = "prop_looking_constrained")
...
```{r}
#looking at time course by morph
...
trial_viz_ci_morph %>% 
  ...
  scale_x_continuous(breaks = seq(from = 0, to = 10, by = 0.5)) +
  scale_size_continuous(breaks = seq(from = 250, to = 0, by = -50), range = c(0.5, 5)) +
  ggthemes::theme_few() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 18),
    plot.subtitle = element_text(hjust = 0.5, size = 12),
...
```{r}
#reverse timecourse visualization broken down by morph
...
```{r}
#making the data frame for all of the models
View(lmer_data)
...
test_data <- 
  ...
  left_join(
    d_trials %>%
      ...
      filter(length > 0) %>% 
      select(SID, trial, accuracy, length),
    by = c("sid" = "SID", "trial")
  )
...
subwindows <- 
  ...
  summarise(mean_looking = mean(looking)) %>% 
  filter(sec <= 5) %>% 
