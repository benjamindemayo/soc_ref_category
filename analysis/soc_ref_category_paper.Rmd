...
knitr::include_graphics(
  path(project_root, "images", "diagram_lit_review_1.jpg")
)
...
```{r}
all_data <- read_csv(processed_data_file)
...
## Participants
I recruited a sample of 37 children aged 3 to 5 years at the Bing Nursery School at Stanford University in Stanford, CA. Seven children were dropped from the analyses because of issues with their data (e.g., incomplete video data or insufficient eye-tracker calibration). One child's data was formatted incorrectly, though I was not able to identify the source of the issue, so I had to drop the participant, leaving me with a final sample size of 29 participants (1 fewer than I specified in my preregistration, which can be found at `https://aspredicted.org/see_one.php?a_id=8960`). Mean age in the final sample was `r mean_age`. I asked all participants if they wanted to play a short game during their regularly scheduled free-play. Children were predominantly white and Asian and generally had highly-educated parents. All children were tested by the same experimenter (myself) and were randomly assigned to one of four counterbalance conditions.
...
knitr::include_graphics(
  path(project_root, "images", "stimuli_1.png")
)
...
knitr::include_graphics(
  path(project_root, "images", "calibration.jpg")
)
...
trial_proportions <- 
  all_data %>% 
  filter(!is.na(trial)) %>% #take only the data that corresponds to a trial
  group_by(sid, trial) %>% 
  mutate(
    trial_length = max(t) - min(t), #make a column for trial length
  ...
  ungroup() %>% 
  count(sid, trial, look, Morph, age, accuracy, block, trial_length) %>% 
  #count amount of measurements where child is looking or not in each trial
  spread(look, n) %>% #now spread it so that that each kid-trial combo is one row (not two)
  replace_na(list(looking = 0, not_looking = 0)) %>% 
  mutate(prop_looking = looking / (looking + not_looking))

#make a data frame where the looking window is constrained
...
trial_proportions_constrained <- 
  ...
  ungroup() %>% 
  count(sid, trial, look, Morph, age) %>% 
  spread(look, n) %>% 
  replace_na(list(looking = 0, not_looking = 0)) %>% 
...
min_length <- mean(test_data$trial_length) - (2 * sd(test_data$trial_length))
max_length <- mean(test_data$trial_length) + (2 * sd(test_data$trial_length))

test_data_trimmed <- 
  test_data %>% 
  filter(trial_length > min_length & trial_length < max_length)
...
```{r forwardtimecourse, fig.cap='Timecourse of proportion looking at experimenter, in half-second intervals. Size of points indicates amount of trials represented in each point. Dashed vertical line is median trial length. Error bars are 95 percent confidence intervals.'}
...
test_data %>% 
  ggplot(aes(as.factor(morph), trial_length)) +
  geom_boxplot() +
...
```{r accuracylookingmorph, fig.cap="Proportion of trial duration spent looking at the experimenter, by morph level and accuracy. When categorizing items of morph level 100 (exact prototypes), children who were inaccurate tended to look at the experimenter more."}
accuracy_morph_n <- 
  trial_proportions %>% 
